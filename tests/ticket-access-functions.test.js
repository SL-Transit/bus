...
(async function run() {
  ...
  assert(index.includes('exports.rotatePassengerTicketToken'), 'Backend must expose trusted ticket token rotation');
  assert(index.includes('exports.revokePassengerTicketToken'), 'Backend must expose trusted ticket token revocation');
  assert(index.includes('nextTicketAccessTokenHash'), 'Rotation must accept only the next raw-token hash');
  assert(!index.includes('nextTicketAccessToken ='), 'Rotation must not store or log the next raw token');
