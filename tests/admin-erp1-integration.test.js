...
const forbiddenConfigTokens = ['databaseURL:', 'firebase.database(', '.ref('].filter((token) => config.includes(token));
if (forbiddenConfigTokens.length) throw new Error(`config must not enable direct database access: ${forbiddenConfigTokens.join(', ')}`);
