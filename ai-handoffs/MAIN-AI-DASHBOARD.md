# มติเจ้าของล่าสุด: หลักการเลือกเส้นทาง ป้าย และจุดต่อรถ — 2026-08-09

## ให้ยึดส่วนนี้ก่อนข้อสรุปเก่าที่ขัดกัน

ส่วนนี้เป็นหลักการออกแบบที่เจ้าของอนุมัติให้ผู้ช่วยทุกตัวใช้ร่วมกัน ยังไม่ใช่คำสั่งให้แก้โค้ด นำเข้าข้อมูล เขียนฐานข้อมูล หรือเผยแพร่ระบบจริง

### ขอบเขตข้อมูลตัวอย่าง

- ชื่อท่ารถ สถานี ป้าย จุดเชื่อมต่อ สาย ผู้ให้บริการ เวลา และราคาที่กล่าวในบทสนทนา เป็นตัวอย่างเพื่ออธิบายวิธีทำงานเท่านั้น
- ห้ามนำชื่อ ความสัมพันธ์ จำนวนกลุ่ม จุดเชื่อมต่อ เวลา หรือราคาจากตัวอย่างไปสร้างเป็นข้อมูลจริง
- ชุดกลุ่มที่ใช้วางรูปแบบหน้าจอ รวมถึงชุดตัวอย่าง 5 กลุ่ม ต้องอ่านจากข้อมูลกลางเมื่อได้รับอนุมัติจริง ห้ามเขียนชื่อหรือจำนวนตายตัวในหน้าเว็บ
- ข้อมูลจริงทุกชุดต้องผ่าน แบบร่าง → ตรวจสอบ → ทบทวน → อนุมัติ → เผยแพร่ แยกต่างหาก

### การจัดสรรข้อมูลกลาง

1. กลุ่มบริการ ใช้จัดหมวดให้ผู้โดยสารเลือก ไม่ใช่สถานที่จริง
2. สถานที่จริง ได้แก่ ท่ารถ สถานี หรือศูนย์เปลี่ยนถ่าย แต่ละแห่งต้องมีรหัสถาวรของตนเอง
3. ป้าย ช่องขึ้นรถ หรือจุดขึ้นลง อยู่ใต้สถานที่จริงและต้องมีรหัสของตนเอง
4. ผู้ให้บริการ สาย และเส้นทางต้องแยกจากสถานที่ ชื่อเหมือนกันไม่ได้แปลว่าเป็นรายการเดียวกัน
5. แบบเส้นทางเก็บลำดับป้ายและแยกตามทิศทาง
6. เที่ยวรถเก็บวันให้บริการ เวลาเริ่ม และสถานะเที่ยว
7. เวลาประจำป้ายเก็บเวลาถึง เวลาออก และลำดับป้ายของเที่ยวเดียวกัน
8. ราคาเก็บตามช่วงต้นทาง–ปลายทาง สาย เที่ยว เงื่อนไขเวลา หรือกฎการต่อรถตามที่ได้รับอนุมัติ
9. กฎการต่อรถเก็บคู่ป้าย คู่สาย หรือคู่เที่ยว เวลาต่อขั้นต่ำ สถานะต่อได้ ต่อแบบรับประกัน หรือห้ามต่อ
...
### พฤติกรรมหน้าผู้โดยสาร

- หน้าจอแสดงกลุ่มบริการจากข้อมูลกลางตามสถานะเปิดใช้งานและลำดับแสดงผล
- เมื่อผู้โดยสารเลือกกลุ่ม ให้แสดงเฉพาะป้ายที่อยู่ในกลุ่มหรือเข้าถึงได้ตามข้อมูลที่เผยแพร่
- เมื่อเลือกต้นทาง ปลายทาง วัน และเวลา ให้แสดงเฉพาะเที่ยวที่วิ่งจริงในวันนั้น และมีลำดับต้นทางอยู่ก่อนปลายทาง
- ถ้ามีเที่ยวตรง ให้แสดงเที่ยวตรง
- ถ้ามีทางต่อ ให้แสดงเฉพาะทางต่อที่ถึงป้ายเชื่อมทันเวลาและไม่ขัดกฎการต่อรถ
- ถ้าไม่มีทางต่อ ให้จบการเดินทางที่ป้ายนั้นและไม่สร้างตัวเลือกขึ้นเอง
- ป้ายเดียวรองรับหลายสายได้ และสายเดียวผ่านหลายป้ายได้
- สถานที่หรือสายที่ชื่อเหมือนกันแต่คนละแห่ง คนละผู้ให้บริการ หรือคนละเส้นทาง ต้องเป็นคนละรหัส

### หลักการค้นหาเส้นทาง

1. ตรวจรหัสต้นทาง ปลายทาง วัน และเวลาที่ผู้โดยสารเลือก
2. กรองปฏิทินและเที่ยวที่เปิดให้บริการ
3. หาเที่ยวตรงจากลำดับป้ายก่อน
4. ถ้าต้องต่อรถ ให้ค้นหาเป็นรอบ โดยแต่ละรอบแทนจำนวนครั้งที่ต่อเพิ่มขึ้น
5. ที่ป้ายเชื่อม ให้คำนวณเวลาพร้อมขึ้นเที่ยวถัดไปจาก เวลาถึง + เวลาเดินเปลี่ยนจุด + เวลาสำรองขั้นต่ำ
6. ปฏิเสธเที่ยวถัดไปที่ออกก่อนเวลาพร้อมต่อรถ ถูกยกเลิก หรือถูกกำหนดว่าห้ามต่อ
7. รวมราคาของแต่ละช่วงและกฎราคาเมื่อต่อรถจากข้อมูลกลาง
8. เรียงผลลัพธ์อย่างน้อยตามเวลาถึง จำนวนครั้งที่ต่อ เวลารอ และราคารวม
9. ส่งผลลัพธ์ที่ผ่านกฎให้หน้าผู้โดยสารแสดง โดยหน้าจอห้ามคำนวณหรือเดาทางต่อเอง

เงื่อนไขเที่ยวตรงขั้นต่ำ:

```text
ลำดับป้ายต้นทาง < ลำดับป้ายปลายทาง
เวลาออกจากต้นทาง >= เวลาที่ผู้โดยสารเลือก
เที่ยวและปฏิทินอยู่ในสถานะให้บริการ
```

เงื่อนไขต่อรถขั้นต่ำ:

```text
เวลาพร้อมขึ้นเที่ยวถัดไป
= เวลาถึงป้ายเชื่อม + เวลาเดินเปลี่ยนจุด + เวลาสำรองขั้นต่ำ
```

### ขอบเขตการเขียนโค้ด

- ศูนย์ข้อมูลกลางเป็นเจ้าของรหัสและความสัมพันธ์ของข้อมูล
- ศูนย์ตรรกะเป็นผู้ตัดสินว่าเที่ยวตรง ทางต่อ และการจองใช้ได้หรือไม่
- ศูนย์คำนวณเป็นผู้คำนวณเวลาเดินทาง เวลารอ ราคา และผลรวมเชิงตัวเลข
- หน้าผู้โดยสารรับผลที่เตรียมแล้วและแสดงเท่านั้น
- ห้ามเขียนรายชื่อกลุ่ม ป้าย จุดเชื่อมต่อ ทางต่อ เวลา หรือราคาไว้ตายตัวในหน้าเว็บ
- ควรใช้แนวทางค้นหาเส้นทางขนส่งแบบเป็นรอบสำหรับข้อมูลขนาดจริง และจำกัดจำนวนครั้งที่ต่อรถตามนโยบายที่เจ้าของอนุมัติ
- ก่อนลงมือเขียนโค้ด ต้องตรวจสัญญาและโครงสร้างปัจจุบันใน GitHub แล้วเสนอขอบเขตไฟล์ การทดสอบ และผลกระทบให้เจ้าของอนุมัติอีกครั้ง

### แหล่งอ้างอิงหลัก

- มาตรฐานตารางเดินรถสาธารณะ: https://gtfs.org/documentation/schedule/reference/
- แนวปฏิบัติการต่อรถ: https://gtfs.org/documentation/schedule/schedule-best-practices/
- แนวคิดสถานที่ ป้าย แบบเส้นทาง และเที่ยวรถ: https://transmodel-cen.eu/index.php/transmodel-at-a-glance/
- วิธีค้นหาเส้นทางแบบเป็นรอบ: https://www.microsoft.com/en-us/research/publication/round-based-public-transit-routing/

สถานะการนำไปใช้: อนุมัติเฉพาะหลักการและการบันทึกเอกสาร ยังไม่มีการอนุมัติให้แก้โค้ด เขียนฐานข้อมูล นำเข้าข้อมูลจริง รวมงาน หรือเผยแพร่ระบบ
