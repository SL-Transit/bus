...
| Passenger bridge | Passenger AI | REVIEW | `passenger.html`, `passenger-logic.js` — **TEMPORARY ROLLBACK**: pointed at the OLD Firebase project (`bus-booking-1d68c`, same as booking.html/check_ticket.html) with old flat schema paths (`settings`/`routeData`/`publishedCatalog`/`bus`/`liveVehicles`), per owner approval, to restore visible map+schedule while sl-transit-9464e has no real config/seed. Not a backbone schema change — booking.html/check_ticket.html/erp-schema.js/erp-data-adapter.js untouched. | 2026-07-05 17:36 +07 | 2026-07-06 06:04 +07 | see `ai-handoffs/CENTRAL-REPORT.md` (06:04 entry) | Revert conditions: (1) real sl-transit-9464e apiKey/appId/messagingSenderId, (2) Data Import AI catalog/fleet/settings seed applied, (3) Main Backbone Lead/Supervisor cutover approval. No Firebase writes, no passenger/private data touched. |
