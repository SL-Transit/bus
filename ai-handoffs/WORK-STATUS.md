...
| Booking1 notification center bridge | Supervisor AI / Project Secretary | DONE | `booking1-preview-adapter.js`, `booking-assignment-center.js`, `functions/index.js`, `functions/staff-notification-center.js`, `functions/driver-ticket-center.js`, notification tests | 2026-07-18 20:55 +07 | 2026-07-18 20:55 +07 | `a8391b1`, `a3ddf9e`, `908beda`, `5e9e3dc`; see CENTRAL-REPORT 2026-07-18 20:55 +07 | Booking1 writes `/bookings/{code}`; ERP Notification Center resolves staff recipients from `/data/notificationCenter/staffLineTargets`; driver assignment enrichment uses `operations/driverWorkByServiceDate` plus ERP Data Center stop aliases. `car1`, `car3/2575`, `car4/2649` ready; `car2` pending LINE user ID. |
...
| Booking1 real seat availability at listing time | Unassigned — requesting Main Backbone Lead / booking-bridge.js owner | BLOCKED | `booking-bridge.js` (`_tripFromTimeEntry`, `decideBookingAvailability` call site), possibly `booking-availability-center.js` | 2026-07-22 12:39 +07 | 2026-07-22 12:39 +07 | see `ai-handoffs/CENTRAL-REPORT.md` (2026-07-22 12:39 entry) | Booking1 trip-list cards need real seats-remaining per trip. `_tripFromTimeEntry()` never passes `capacity`/`bookedSeats` into `decideBookingAvailability()`, so `availabilityDecision.seatsAvailable` is always `null` at listing time (booking1 correctly hides the seats row rather than fabricate a number). The `Booking1 capacity transaction` work (`582ed32`) already writes real counts to `operations/bookingCapacityByServiceDate/{serviceDate}/{capacityKey}` but only reads/writes them at booking-confirm time, not at listing time. Requesting this counter be read (or an equivalent capacity summary be exposed) when building each trip so `seatsAvailable` reflects reality in the list, not just at confirm. Booking1 side (`booking1-preview-adapter.js`) is already ready to render it once populated (shows "ที่นั่งเต็มแล้ว" + disables booking automatically when `seatsAvailable === 0`). Flagged instead of implemented directly because `booking-bridge.js` is being actively developed by another AI (capacity-transaction work) and adding a live per-trip Firebase read on every list render is a real design decision (read strategy/caching), not a cosmetic change. |
