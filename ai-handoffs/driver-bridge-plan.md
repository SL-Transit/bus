...
1. ~~`operations/liveVehicles` record shape is undefined.~~ **RESOLVED** — Main Backbone Lead added `liveVehicle: ['vehicleId','lat','lng','updatedAt','serviceStatus']` plus `VALID_LIVE_VEHICLE_STATUS` and reference validation for `vehicleId`/`queueId`/`currentTripId` (seen in `erp-schema.js` as of this update). Bridge plan §5 updated accordingly below.
