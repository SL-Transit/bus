...
function authorizationError(code) { const error = new Error(code); error.code = code; error.httpStatus = code === "unauthenticated" ? 401 : 403; return error; }
function safeUid(uid) { return typeof uid === "string" && uid.length > 0 && uid.length <= 128 && !/[.#$[\]/]/.test(uid); }

function packageOperatorIds(payload) {
  if (payload && Array.isArray(payload.operatorScope)) return Array.from(new Set(payload.operatorScope));
  const pkg = payload && payload.package;
  const values = pkg && pkg.metadata && Array.isArray(pkg.metadata.operatorScope) ? pkg.metadata.operatorScope : [];
...
function hasOperatorScope(account, operatorIds) {
  ...
  return Array.isArray(operatorIds) && operatorIds.length > 0 && operatorIds.every(function (operatorId) { return configured.includes(operatorId); });
}
...
async function authorizeCommand(input) {
  ...
  if (!options.accessReader || typeof options.accessReader.getAccount !== "function") throw new Error("greenfield_access_reader_required");
  ...
  if (policy.operatorScoped && !hasOperatorScope(account, packageOperatorIds(options.payload))) throw authorizationError("operator_scope_denied");
  return Object.freeze({ uid: options.uid, role: options.role, account });
}

module.exports = { COMMAND_POLICIES, authorizeCommand, hasOperatorScope, packageOperatorIds, safeUid };