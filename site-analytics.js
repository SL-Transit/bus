(function(){
  'use strict';
  var CONSENT_EVENT='sltransit:consent-changed';
  function hasConsent(){return !!(window.SLTransitConsent&&window.SLTransitConsent.hasAnalyticsConsent&&window.SLTransitConsent.hasAnalyticsConsent());}
  function waitForConsent(){if(hasConsent())return start();if(window.SLTransitConsent&&window.SLTransitConsent.hasDecided&&window.SLTransitConsent.hasDecided())return;window.addEventListener(CONSENT_EVENT,function(e){if(e&&e.detail&&e.detail.analytics===true)start();},{once:true});}
  ...
  function start(){
    if(start.done||!hasConsent())return;
