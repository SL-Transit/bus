...
public class GpsService extends Service {
    ...
    private static final long RESTART_INTERVAL_MS = 5 * 60 * 1000; // 5 นาที
    ...
    private void startTracking() {
        initFirebase();
        acquireWakeLock();
        scheduleRestart(); // ตั้ง alarm ไว้เผื่อถูก kill
        if (running) {
    ...
    private void stopTracking() {
        ...
        markOffline();
        releaseWakeLock();
        cancelRestart(); // ยกเลิก alarm เมื่อหยุดเอง
        stopForeground(true);
        stopSelf();
    }
    ...
    private void scheduleRestart() {
        try {
            ...
            Log.e(TAG, "scheduleRestart failed: " + e.getMessage());
        }
    }
    ...
    // เรียกเมื่อผู้ใช้ swipe แอปออกจาก Recent Apps
    @Override public void onTaskRemoved(Intent rootIntent) {
        super.onTaskRemoved(rootIntent);
        SharedPreferences p = getSharedPreferences(MainActivity.PREFS, MODE_PRIVATE);
        if (p.getBoolean(MainActivity.KEY_ENABLED, false)) {
            scheduleRestart(); // restart ใน 5 นาที
        }
    }
