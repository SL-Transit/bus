...
public class GpsService extends Service implements SensorEventListener {
    ...
    private static final long GPS_REREGISTER_MS     = 180000; // ข้อ 3: re-register เมื่อ callback หายเกิน 3 นาที
    private static final long GPS_PENDING_INTENT_MS = 300000; // ข้อ 4: เปิด PendingIntent recovery เมื่อเกิน 5 นาที
    private android.app.PendingIntent locationPendingIntent = null;
    private int gpsLostCount      = 0; // ข้อ 6: สถิติ GPS Lost
    private int gpsRecoveryCount  = 0; // ข้อ 6: สถิติ GPS Recovery
    private String lastLocationSource = "none"; // ข้อ 5: source ของ location ล่าสุด
    ...
    // Driver Work Center เป็นเจ้าของคิว/งานประจำวัน ส่วน GPS service ไม่คำนวณคิวเอง
    private int centralQueueNo = -1;
    ...
    private final Runnable gpsWatchdog = new Runnable() {
        @Override public void run() {
            ...
            if (referenceAt > 0) {
                ...
                // บันทึกสถานะทุกรอบเพื่อดูว่า WakeLock หลุดหรือ callback ไม่มาเลย
                if (gpsAgoMs > 30000) {
                    logDiagToFirebase("gps_watchdog_check",
                        "gpsAgoSec=" + (gpsAgoMs / 1000)
                        + " wakelockHeld=" + (wakeLock != null && wakeLock.isHeld())
                        + " callbackRegistered=" + (currentGpsRequestMs > 0)
                        + " lastGpsUpdateAt=" + (lastGpsUpdateAt > 0 ? "received" : "never")
                        + " mode=" + trackingMode
                        + " source=" + lastLocationSource
                        + " lostCount=" + gpsLostCount
                        + " recoveryCount=" + gpsRecoveryCount
                        + " recovering=" + (now - lastGpsRecoveryAt < GPS_RECOVERY_RETRY_MS));
                }
                if (gpsAgoMs > staleThreshold && now - lastGpsRecoveryAt >= GPS_RECOVERY_RETRY_MS) {
                    // ข้อ 3: เกิน 180s — re-register LocationCallback (Primary recovery)
                    if (gpsAgoMs <= GPS_PENDING_INTENT_MS) {
                        recoverGpsProvider(gpsAgoMs);
                    }
                    // ข้อ 4: เกิน 300s — เปิด PendingIntent recovery เป็น fallback
                    else if (gpsAgoMs > GPS_PENDING_INTENT_MS) {
    ...
    private void recoverGpsProvider(long gpsAgoMs) {
        ...
        gpsLostCount++;
        Log.w(TAG, "GPS หาย " + (gpsAgoMs / 1000) + "s — re-register LocationCallback [lost#" + gpsLostCount + "]");
        recordStatus("recovering_gps");
        logDiagToFirebase("gps_recovery_triggered",
            "gpsAgoSec=" + (gpsAgoMs / 1000) + " mode=" + trackingMode
            + " method=reregister_callback"
            + " lostCount=" + gpsLostCount
            + " restartCount=" + prefs.getInt(MainActivity.KEY_RESTART_COUNT, 0));
        updateNotification("กำลังกู้คืนสัญญาณ GPS... [" + queueId + "]");
        // ยกเลิก PendingIntent เก่าถ้ามี
        cancelLocationPendingIntent();
        try {
        ...
        try {
            ...
            fusedClient.getCurrentLocation(Priority.PRIORITY_HIGH_ACCURACY, tokenSource.getToken())
                    .addOnSuccessListener(loc -> {
                        if (loc != null) {
                            loc.setProvider("one_shot_recovery"); // ข้อ 5: บันทึก source
                            handleLocationResult(loc);
                        }
                        gpsRecoveryCount++;
                        logDiagToFirebase("gps_recovery_success",
                            "method=reregister_callback recoveryCount=" + gpsRecoveryCount);
                    })
                    .addOnFailureListener(error -> {
                        logDiagToFirebase("gps_recovery_failed",
                            "method=reregister_callback error=" + error.getMessage());
                        recordError("GPS recovery failed: " + error.getMessage());
                    });
        } catch (Exception error) {
            logDiagToFirebase("gps_recovery_failed", "exception=" + error.getMessage());
            recordError("GPS recovery failed: " + error.getMessage());
        }
    }

    // ข้อ 4: PendingIntent Recovery — fallback เมื่อ LocationCallback หยุดทำงานนานเกิน 5 นาที
    // รองรับ Android 10-16 โดยไม่ผูกกับยี่ห้อใด
    private void recoverWithPendingIntent(long gpsAgoMs) {
        lastGpsRecoveryAt = System.currentTimeMillis();
        Log.w(TAG, "GPS หาย " + (gpsAgoMs / 1000) + "s — เปิด PendingIntent recovery");
        logDiagToFirebase("gps_pendingintent_recovery",
            "gpsAgoSec=" + (gpsAgoMs / 1000) + " mode=" + trackingMode
            + " lostCount=" + gpsLostCount);
        updateNotification("กู้คืน GPS (PendingIntent)... [" + queueId + "]");
        try {
            // ยกเลิก request เก่าทั้งหมดก่อน
            if (fusedClient != null) fusedClient.removeLocationUpdates(fusedCallback);
            cancelLocationPendingIntent();
            fusedClient = LocationServices.getFusedLocationProviderClient(GpsService.this);
            currentGpsRequestMs = 0;
            ...
            // สร้าง PendingIntent ที่ชี้ไปยัง BroadcastReceiver ใน GpsService
            Intent intent = new Intent(GpsService.this, GpsService.LocationUpdateReceiver.class);
            locationPendingIntent = PendingIntent.getBroadcast(
                GpsService.this, 200, intent,
                Build.VERSION.SDK_INT >= 23
                    ? PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE
                    : PendingIntent.FLAG_UPDATE_CURRENT);

            // ข้อ 7: ใช้ LocationRequest.Builder รองรับ Android 10+ (API 29+)
            LocationRequest piReq = new LocationRequest.Builder(
            ...
            fusedClient.requestLocationUpdates(piReq, locationPendingIntent);
            logDiagToFirebase("gps_pendingintent_registered",
                "interval=10000 priority=HIGH_ACCURACY");

            // หลัง 60 วินาที กลับมาใช้ LocationCallback ถ้า GPS กลับมาแล้ว
            handler.postDelayed(() -> {
                if (!running) return;
                cancelLocationPendingIntent();
                if (lastGpsUpdateAt > lastGpsRecoveryAt) {
                    // GPS กลับมาแล้ว — สลับกลับเป็น callback
                    gpsRecoveryCount++;
                    logDiagToFirebase("gps_pendingintent_success",
                        "recoveryCount=" + gpsRecoveryCount);
                }
                configureLocationRequests(true);
            }, 60000);
            ...
            logDiagToFirebase("gps_pendingintent_failed", "SecurityException: " + e.getMessage());
            recoverGpsProvider(gpsAgoMs); // fallback กลับไป re-register
        } catch (Exception e) {
            logDiagToFirebase("gps_pendingintent_failed", "Exception: " + e.getMessage());
            recoverGpsProvider(gpsAgoMs);
        }
    }

    private void cancelLocationPendingIntent() {
        if (locationPendingIntent != null && fusedClient != null) {
            try { fusedClient.removeLocationUpdates(locationPendingIntent); } catch (Exception ignored) {}
    ...
    private void handleLocationResult(Location location) {
        ...
        if (filtered == null) {
            // นับจำนวนครั้งที่ callback มาแต่ถูก filter ทิ้ง
            // ถ้า count > 0 = chip ยังส่ง fix อยู่แต่ accuracy เลว
            // ถ้า count = 0 ตอน GPS หาย = callback ไม่มาเลย (FLP หยุดทำงาน)
            int fc = prefs.getInt(MainActivity.KEY_LOCATION_FILTER_COUNT, 0);
            prefs.edit().putInt(MainActivity.KEY_LOCATION_FILTER_COUNT, fc + 1).apply();
            logDiagToFirebase("location_filtered_out",
                "accuracy=" + (location.hasAccuracy() ? location.getAccuracy() : -1)
                + " provider=" + location.getProvider()
                + " ageMs=" + (System.currentTimeMillis() - location.getTime()));
            return;
        }
        // ข้อ 5: บันทึก source ของ location ที่ผ่าน filter
        lastLocationSource = location.getProvider() != null ? location.getProvider() : "unknown";
        latestLocation = filtered;
        saveCoords(filtered);
        if (!filtered.hasAccuracy() || filtered.getAccuracy() <= 50f) {
            lastGpsFixLocation = new Location(filtered);
        }
        // รีเซ็ต filter count เมื่อได้ fix จริง และอัปเดต wakelock status
        prefs.edit()
            .putInt(MainActivity.KEY_LOCATION_FILTER_COUNT, 0)
            .putBoolean(MainActivity.KEY_WAKELOCK_HELD, wakeLock != null && wakeLock.isHeld())
            .putBoolean(MainActivity.KEY_CALLBACK_REGISTERED, true)
            .apply();
    ...
    // ===== รับคิวจาก Driver Work Center เท่านั้น =====
    private void loadQueueSchedule() {
        centralQueueNo = prefs.getInt(MainActivity.KEY_DRIVER_QUEUE_NO, -1);
        scheduleLoaded = true;
    }
    ...
    private void startTracking() {
        ...
        // ลงทะเบียน receiver สำหรับ PendingIntent location updates
        android.content.IntentFilter locFilter = new android.content.IntentFilter("com.sanamchai.drivergps.LOCATION_UPDATE");
        if (Build.VERSION.SDK_INT >= 33) {
            registerReceiver(locationUpdateReceiver, locFilter, android.content.Context.RECEIVER_NOT_EXPORTED);
        } else {
            registerReceiver(locationUpdateReceiver, locFilter);
        }
        ...
        String queueLabel = centralQueueNo > 0 ? " คิว" + centralQueueNo : "";
    ...
    private void configureLocationRequests(boolean force) {
        ...
        // บันทึกสถานะ wakelock + callback ทุกครั้งที่ re-register
        prefs.edit()
            .putBoolean(MainActivity.KEY_WAKELOCK_HELD, wakeLock != null && wakeLock.isHeld())
            .putBoolean(MainActivity.KEY_CALLBACK_REGISTERED, true)
            .putString(MainActivity.KEY_LAST_REQUEST_ERROR, "")
            .apply();
        logDiagToFirebase("location_request_registered",
            "priority=" + priority + " intervalMs=" + intervalMs
            + " mode=" + trackingMode + " force=" + force);
        try {
            fusedClient.removeLocationUpdates(fusedCallback);
            logDiagToFirebase("location_updates_removed",
                "caller=configureLocationRequests mode=" + trackingMode
                + " wakelockHeld=" + (wakeLock != null && wakeLock.isHeld())
                + " gpsAgoSec=" + (lastGpsUpdateAt > 0 ? (System.currentTimeMillis() - lastGpsUpdateAt) / 1000 : -1));
            ...
            logDiagToFirebase("location_updates_registered",
                "intervalMs=" + intervalMs + " priority=" + priority
                + " wakelockHeld=" + (wakeLock != null && wakeLock.isHeld())
                + " — waiting for first callback");
            logBatteryMode("location_request");
        } catch (SecurityException e) {
            prefs.edit().putBoolean(MainActivity.KEY_CALLBACK_REGISTERED, false)
                .putString(MainActivity.KEY_LAST_REQUEST_ERROR, "SecurityException: " + e.getMessage()).apply();
            logDiagToFirebase("location_request_failed", "SecurityException: " + e.getMessage());
            recordError(e.getMessage());
        } catch (Exception e) {
            prefs.edit().putBoolean(MainActivity.KEY_CALLBACK_REGISTERED, false)
                .putString(MainActivity.KEY_LAST_REQUEST_ERROR, "Exception: " + e.getMessage()).apply();
            logDiagToFirebase("location_request_failed", "Exception: " + e.getMessage());
    ...
    private Map<String, Object> buildStatusData(Location loc, boolean online,
        ...
        if (centralQueueNo > 0) {
            data.put("queue",        centralQueueNo);
            data.put("queueNo",      centralQueueNo);
            data.put("todayQueueNo", centralQueueNo);
        } else {
            data.put("queue",        null);
    ...
    // ===== Diagnostic logger — บันทึก event เชิงเทคนิคสำหรับ admin วิเคราะห์ GPS หาย =====
    ...
    public static class LocationUpdateReceiver extends android.content.BroadcastReceiver {
        @Override
        public void onReceive(android.content.Context context, Intent intent) {
            if (!LocationResult.hasResult(intent)) return;
            LocationResult result = LocationResult.extractResult(intent);
            if (result == null) return;
            // ส่ง broadcast ให้ GpsService หลัก process location
            Intent fwd = new Intent("com.sanamchai.drivergps.LOCATION_UPDATE");
            Location best = null;
            for (Location loc : result.getLocations()) {
                if (best == null || (loc.hasAccuracy() && (!best.hasAccuracy() || loc.getAccuracy() < best.getAccuracy())))
                    best = loc;
            }
            if (best != null) {
                fwd.putExtra("lat", best.getLatitude());
                fwd.putExtra("lng", best.getLongitude());
                fwd.putExtra("accuracy", best.hasAccuracy() ? best.getAccuracy() : -1f);
                fwd.putExtra("time", best.getTime());
                fwd.putExtra("source", "pending_intent");
                context.sendBroadcast(fwd);
            }
        }
    }

    // รับ broadcast จาก LocationUpdateReceiver
    private final android.content.BroadcastReceiver locationUpdateReceiver = new android.content.BroadcastReceiver() {
        @Override
        public void onReceive(android.content.Context context, Intent intent) {
            if (!"com.sanamchai.drivergps.LOCATION_UPDATE".equals(intent.getAction())) return;
            Location loc = new Location("pending_intent"); // ข้อ 5: source = pending_intent
            loc.setLatitude(intent.getDoubleExtra("lat", 0));
            loc.setLongitude(intent.getDoubleExtra("lng", 0));
            loc.setAccuracy(intent.getFloatExtra("accuracy", 50f));
            loc.setTime(intent.getLongExtra("time", System.currentTimeMillis()));
            handleLocationResult(loc);
        }
    };
