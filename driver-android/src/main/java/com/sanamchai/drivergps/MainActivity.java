...
public class MainActivity extends Activity {
    ...
    private static final String[] NAV_LABELS = {"หน้าหลัก", "งานวันนี้", "รายงาน", "แจ้งเตือน", "บัญชี"};
    ...
    private void updateReadinessCard(long gpsAgoSec, long fbAgoSec) {
        ...
        readinessBadge.setText(ready ? "พร้อม" : "ไม่พร้อม");
    ...
    private void buildUi() {
        ...
        pickerLp.setMargins(0, dp(6), 0, dp(20));
        ...
        queueTopRow.setClickable(true);
        queueTopRow.setOnClickListener(v -> {
            animateTap(queueTopRow);
            showQueuePickerDialog();
        });
        ...
        queueTopRow.addView(queueValueText);
        ...
        readinessBadge.setText("ไม่พร้อม");
        ...
        // ===== ข้อ 5: ตารางไอคอน 3x2 — เริ่มงาน / นำทาง / รับ-ส่งผู้โดยสาร / ตรวจตั๋ว / ติดตามตำแหน่ง / แจ้งปัญหา =====
        ...
        actionsRowTop.addView(buildStartWorkButton());
        actionsRowTop.addView(buildActionButton("🧭", "นำทาง", COLOR_OCEAN, v -> openExternalNavigation()));
        actionsRowTop.addView(buildActionButton("👥", "รับ-ส่ง\nผู้โดยสาร", COLOR_TEAL, v -> showPassengerList()));
        ...
        actionsRowBottom.addView(buildActionButton("🎫", "ตรวจตั๋ว", COLOR_ORANGE, v -> openQrScanner()));
        actionsRowBottom.addView(buildActionButton("📍", "ติดตาม\nตำแหน่ง", COLOR_LIGHT_TEAL, v -> scrollToTravelCard()));
    ...
    // ===== ข้อ 5: ปุ่ม "เริ่มงาน/หยุดงาน" แบบไอคอน — เหมือน buildActionButton แต่เก็บ ref ไว้สลับสถานะได้ =====
    private LinearLayout buildStartWorkButton() {
        startWorkButton = new LinearLayout(this);
        startWorkButton.setOrientation(LinearLayout.VERTICAL);
        startWorkButton.setGravity(Gravity.CENTER);
        startWorkButton.setPadding(dp(10), dp(16), dp(10), dp(14));
        GradientDrawable bg = new GradientDrawable();
        bg.setColor(Color.WHITE);
        bg.setCornerRadius(dp(16));
        startWorkButton.setBackground(bg);
        startWorkButton.setElevation(dp(1));
        ...
        startWorkLabel.setTextColor(COLOR_NAVY);
        startWorkLabel.setTextSize(12);
        startWorkLabel.setTypeface(Typeface.DEFAULT_BOLD);
        startWorkLabel.setGravity(Gravity.CENTER);
        startWorkLabel.setPadding(0, dp(8), 0, 0);
        startWorkButton.addView(startWorkLabel);

        LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1f);
        lp.setMargins(dp(5), dp(5), dp(5), dp(5));
        startWorkButton.setLayoutParams(lp);
    ...
    private LinearLayout buildBottomNavBar() {
        ...
        for (int i = 0; i < NAV_LABELS.length; i++) {
            ...
            LinearLayout.LayoutParams tabLp = new LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1f);
    ...
    private void selectNavTab(int index) {
        ...
        for (int i = 0; i < navTabs.length; i++) {
            ...
            int color = active ? COLOR_TEAL : COLOR_TEXT_MUTED;
    ...
    private void setStartWorkVisual(boolean isWorking, String label) {
        ...
        startWorkIconBg.setColor(isWorking ? COLOR_RED : COLOR_GREEN);
        startWorkIcon.setBackground(startWorkIconBg);
