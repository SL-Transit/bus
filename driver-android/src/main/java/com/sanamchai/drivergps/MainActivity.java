...
public class MainActivity extends Activity {
    ...
    static final String KEY_BG_LOCATION_PROMPTED = "bg_location_prompted";
    ...
    private static final int REQ_LOCATION_NOTIF      = 10;
    private static final int REQ_CAMERA              = 30;
    private static final int REQ_BACKGROUND_LOCATION = 11;
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        if (prefs.getBoolean(KEY_ENABLED, false)) {
            refreshUi();
            // กำลังส่ง GPS อยู่แล้วจาก session ก่อนหน้า — เช็คอีกรอบว่ายังมีสิทธิ์
            // "อนุญาตตลอดเวลา" อยู่ไหม (เคสที่พบบ่อย: ผู้ใช้กดปิดสิทธิ์ทีหลัง หรือ
            // เครื่องบางรุ่นรีเซ็ตสิทธิ์ background location เองตอนไม่ได้เปิดแอปนานๆ)
            uiHandler.postDelayed(() -> maybeRequestBackgroundLocation(false), 1500);
        } else {
        ...
        try {
            ...
            BroadcastReceiver receiver = new BroadcastReceiver() {
                @Override public void onReceive(Context context, Intent intent) {
                    ...
                    // เดิมโค้ดเรียก installApk() ทันทีโดยไม่เช็คว่าดาวน์โหลดสำเร็จจริงไหม
                    // broadcast นี้ยิงทุกครั้งไม่ว่าจะสำเร็จหรือพัง (เช่น ลิงก์ apkUrl
                    // เป็นหน้า HTML/ต้อง login แทนไฟล์ apk จริง) ทำให้พยายามติดตั้งไฟล์เสีย
                    checkDownloadResultThenInstall(dm, downloadId, outFile);
                }
            };
    ...
    // ===== เช็คผลดาวน์โหลดจริงจาก DownloadManager ก่อนสั่งติดตั้ง =====
    private void checkDownloadResultThenInstall(DownloadManager dm, long downloadId, File outFile) {
        try (android.database.Cursor cursor = dm.query(new DownloadManager.Query().setFilterById(downloadId))) {
            if (cursor == null || !cursor.moveToFirst()) {
                showInstallError("ไม่พบข้อมูลการดาวน์โหลด กรุณาลองใหม่");
                return;
            }
            int status = cursor.getInt(cursor.getColumnIndexOrThrow(DownloadManager.COLUMN_STATUS));
            if (status != DownloadManager.STATUS_SUCCESSFUL) {
                int reason = cursor.getInt(cursor.getColumnIndexOrThrow(DownloadManager.COLUMN_REASON));
                showInstallError("ดาวน์โหลดไม่สำเร็จ (เหตุผล: " + reason + ")\n\n" +
                        "ลิงก์ apkUrl อาจไม่ใช่ไฟล์ apk ที่ดาวน์โหลดได้ตรงๆ " +
                        "(เช่น เป็นลิงก์ GitHub Actions artifact ที่ต้อง login — " +
                        "ต้องใช้ลิงก์จาก GitHub Release แทน) กรุณาตรวจสอบที่หน้า admin");
                return;
            }
            if (!outFile.exists() || outFile.length() < 1024) {
                showInstallError("ไฟล์ที่ดาวน์โหลดมามีขนาดเล็กผิดปกติ (" + outFile.length() +
                        " bytes) — น่าจะไม่ใช่ไฟล์ apk จริง กรุณาตรวจสอบลิงก์ apkUrl ที่หน้า admin");
                return;
            }
            installApk(outFile);
        } catch (Exception e) {
    ...
    private void installApk(File apkFile) {
        // Android 8+ ต้องได้รับอนุญาต "ติดตั้งแอปที่ไม่รู้จัก" สำหรับแอปนี้โดยเฉพาะก่อน
        // ไม่งั้นระบบจะบล็อกเงียบๆ หรือเด้งไปหน้า Settings เองโดยไม่อธิบายอะไร
        if (Build.VERSION.SDK_INT >= 26 && !getPackageManager().canRequestPackageInstalls()) {
            new AlertDialog.Builder(this)
                    .setTitle("📦 ต้องอนุญาตติดตั้งแอปก่อน")
                    .setMessage("กรุณากด \"อนุญาต\" ในหน้าตั้งค่าที่จะเปิดขึ้น เพื่อให้ติดตั้งไฟล์อัพเดทได้")
                    .setCancelable(false)
    ...
    private void buildUi() {
        ...
        diagPanel.setOnClickListener(v -> {
            // กดที่แผงวินิจฉัยเพื่อ "แก้ไข" ตามที่ข้อความในแผงสื่อไว้ —
            // เช็คทั้ง battery optimization และ background location แล้วพาไปตั้งค่าที่ขาด
            boolean batteryOk = true;
            if (Build.VERSION.SDK_INT >= 23) {
                PowerManager pm = (PowerManager) getSystemService(POWER_SERVICE);
                batteryOk = pm != null && pm.isIgnoringBatteryOptimizations(getPackageName());
            }
            if (!batteryOk) {
                prefs.edit().putBoolean(KEY_BATTERY_PROMPTED, false).apply();
                requestBatteryUnrestrictedIfNeeded();
            } else {
                maybeRequestBackgroundLocation(true);
            }
        });
    ...
    private void requestPermissionsThenStart() {
        ...
        else requestPermissions(permissions.toArray(new String[0]), REQ_LOCATION_NOTIF);
    }

    @Override public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grants) {
        super.onRequestPermissionsResult(requestCode, permissions, grants);
        if (requestCode == REQ_LOCATION_NOTIF && hasLocationPermission()) {
            startGpsService();
            // ขอสิทธิ์ "อนุญาตตลอดเวลา" เป็นขั้นที่ 2 แยกจาก fine location
            // (Android 10+ ไม่อนุญาตให้ขอ background location พร้อมกับ foreground ในครั้งเดียว)
            uiHandler.postDelayed(() -> maybeRequestBackgroundLocation(false), 800);
        }
        else if (requestCode == REQ_BACKGROUND_LOCATION) refreshUi();
    ...
    // เดิมโค้ดขอแค่ ACCESS_FINE_LOCATION ตอนเปิดแอปครั้งแรก ทำให้ระบบให้สิทธิ์
    // แบบ "อนุญาตเฉพาะตอนใช้งาน" เท่านั้น พอแอป background/จอดับ Android จะตัด
    // การเข้าถึง GPS ทันที (Service ยังวิ่งอยู่ ส่ง heartbeat เข้า Firebase ได้
    // ตามปกติ แต่ขอพิกัดใหม่ไม่ได้ — ตรงกับอาการ "GPS หาย" ที่พิกัดค้าง)
    //
    // manualTrigger = true เมื่อผู้ใช้กดเองจากแผงวินิจฉัย (ไม่เช็คว่าเคย prompt ไปแล้วหรือยัง)
    private void maybeRequestBackgroundLocation(boolean manualTrigger) {
        if (Build.VERSION.SDK_INT < 29) return; // background location ไม่มีก่อน Android 10
        boolean hasBg = checkSelfPermission(Manifest.permission.ACCESS_BACKGROUND_LOCATION)
                == PackageManager.PERMISSION_GRANTED;
        if (hasBg) return;
        if (!hasLocationPermission()) return; // ต้องได้ fine/coarse location ก่อนถึงจะขอ background ได้
        if (!manualTrigger && prefs.getBoolean(KEY_BG_LOCATION_PROMPTED, false)) return;
        if (isFinishing()) return;

        prefs.edit().putBoolean(KEY_BG_LOCATION_PROMPTED, true).apply();

        new AlertDialog.Builder(this)
                .setTitle("📍 ต้องอนุญาตตำแหน่ง \"ตลอดเวลา\"")
                .setMessage("เพื่อให้ระบบส่งพิกัด GPS ต่อเนื่องได้แม้ปิดหน้าแอปหรือจอดับ " +
                        "กรุณาเลือกสิทธิ์ตำแหน่งเป็น \"อนุญาตตลอดเวลา\" (Allow all the time)\n\n" +
                        "ถ้าเลือกแค่ \"อนุญาตเฉพาะตอนใช้งาน\" ระบบปฏิบัติการจะตัดสัญญาณ GPS " +
                        "ทันทีที่สลับหน้าหรือจอดับ ทำให้พิกัดค้างและขึ้นว่า \"GPS หาย\"")
                .setCancelable(false)
                .setPositiveButton("ไปตั้งค่า", (d, w) -> openBackgroundLocationSetting())
                .setNegativeButton("ข้ามไปก่อน", null)
                .show();
    }

    private void openBackgroundLocationSetting() {
        try {
            if (Build.VERSION.SDK_INT == 29) {
                // Android 10: ระบบยังโชว์ป๊อปอัป 3 ตัวเลือกปกติได้ (While using / All the time / Deny)
                requestPermissions(new String[]{Manifest.permission.ACCESS_BACKGROUND_LOCATION},
                        REQ_BACKGROUND_LOCATION);
            } else {
                // Android 11+: requestPermissions() จะไม่โชว์ตัวเลือก "ตลอดเวลา" ให้ในป๊อปอัปอีกต่อไป
                // (ข้อกำหนดของ Google) ต้องพาไปหน้า Settings ของแอปเพื่อเลือกเอง
