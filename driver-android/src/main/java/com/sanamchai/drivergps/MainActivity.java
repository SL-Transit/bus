...
"
        ...
        // แจ้งเตือนถ้ามีปัญหา
        String error = "";
        if (gpsAgoSec > 90) error += "⚠ GPS หาย กรุณาตรวจสอบสัญญาณ\n";
        if (fbAgoSec > 90)  error += "⚠ Firebase ขาดการเชื่อมต่อ ตรวจสอบอินเทอร์เน็ต\n";
        if (restartCount >= 3) error += "⚠ แอปถูก kill บ่อย ตรวจสอบการตั้งค่า Battery\n";
        if (!error.isEmpty()) {
            errorText.setText(error.trim());
